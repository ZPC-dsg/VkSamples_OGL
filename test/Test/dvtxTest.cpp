...
void DvtxTest::prepare() {
	...
	Dynamic::Dsr::ShaderReflection reflection(m_shader);
	std::vector<Dynamic::Dsr::VertexAttrib> attribs = reflection.GetVertexAttribs();
	...
	m_layout = std::make_shared<Bind::InputLayout>(std::vector<std::shared_ptr<Bind::VertexBuffer>>{ vertex_buffer }, index_buffer);
	m_shader->use();
}
